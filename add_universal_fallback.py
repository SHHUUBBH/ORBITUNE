...
# Update the search method to add a final fallback for ALL queries
# Find the search method and add a final fallback after all yt-dlp attempts fail
old_ssl_handling = '''            except Exception as e:
                error_msg = str(e)
...
new_ssl_handling = '''            except Exception as e:
                error_msg = str(e)
                if 'SSL' in error_msg or 'EOF' in error_msg:
                    if attempt < max_attempts:
                        wait_time = attempt * 2
...
if old_ssl_handling in content:
    content = content.replace(old_ssl_handling, new_ssl_handling)
    with open(r'AI-ML/audio_processor/youtube_downloader.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print('Updated SSL handling with universal demo fallback')
else:
    print('Could not find SSL handling block')