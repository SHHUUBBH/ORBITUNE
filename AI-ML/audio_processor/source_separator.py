...
class SourceSeparator:
    ...
    def __init__(self, model_name: str = DEMUCS_MODEL):
        ...
        print("="*70)
        print("[SEP] ORBITUNE - Source Separator")
        print("="*70)
        print(f"[SEP] Model: {model_name}")
        print(f"[SEP] Device: {self.device}")
        
        if USE_GPU:
            print(f"[SEP] GPU Memory: {GPU_MEMORY_GB:.2f} GB")
            print(f"[SEP] Quality: Maximum ({DEMUCS_SHIFTS} shifts)")
        else:
            print("[WARN] Using CPU (slower but works)")
        
        # Load Demucs model
        print("\n[SEP] Loading Demucs AI model...")
        ...
        print("="*70)
        print("[OK] Source Separator Ready!")
        print("="*70)
    ...
    def separate(self, song_id: str, progress_callback=None) -> Optional[Dict[str, Path]]:
        ...
        """
        print(f"\n{'='*70}")
        print(f"[SEP] SEPARATING SONG: {song_id}")
        print(f"{'='*70}")
        ...
        print(f"[SEP] Input: {input_path.name}")
        print(f"[SEP] Output: {output_dir}")
        print(f"[SEP] File size: {input_path.stat().st_size / (1024*1024):.1f} MB")
        
        try:
            # Step 1: Load audio
            print("\n[SEP] Step 1/4: Loading audio...")
            audio, sr = self._load_audio(input_path)
            print(f"   [OK] Loaded: {audio.shape[0]} channels, {audio.shape[1]} samples")
            print(f"   [SEP] Duration: {audio.shape[1] / sr:.1f} seconds")
            
            if progress_callback:
                progress_callback(25, "Audio loaded")
            
            # Step 2: Preprocess audio
            print("\n[SEP] Step 2/4: Preprocessing audio...")
            audio = self._preprocess_audio(audio, sr)
            print(f"   [OK] Preprocessed: {audio.shape}")
            
            if progress_callback:
                progress_callback(35, "Audio preprocessed")
            
            # Step 3: AI Separation (THE MAGIC!)
            print("\n[SEP] Step 3/4: AI Separation (this is where the magic happens)...")
            print(f"   [SEP] Using {DEMUCS_SHIFTS} shifts for maximum quality")
            print(f"   [SEP] Processing on {self.device}...")
            
            start_time = time.time()
            stems = self._separate_stems(audio)
            separation_time = time.time() - start_time
            
            print(f"   [OK] Separation complete in {separation_time:.1f} seconds!")
            print(f"   [SEP] Separated into {len(stems)} stems")
            
            if progress_callback:
                progress_callback(75, "Separation complete")
            
            # Step 4: Save stems
            print("\n[SEP] Step 4/4: Saving stems...")
            ...
            print(f"{'='*70}")
            print(f"[SEP] Total time: {separation_time:.1f} seconds")
            print(f"[SEP] Output directory: {output_dir}")
            print("\n[SEP] Stems created:")
            for stem_name, path in output_paths.items():
                size_mb = path.stat().st_size / (1024*1024)
                print(f"   [OK] {stem_name:8s}: {path.name:20s} ({size_mb:.1f} MB)")
            print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("[SEP] ORBITUNE - Source Separator Test")
    print("="*70)
    ...
    if not downloaded_songs:
        ...
        print(f"\n[SEP] Found {len(downloaded_songs)} downloaded song(s):")
        ...
        if choice.isdigit() and 1 <= int(choice) <= len(downloaded_songs):
            ...
            print(f"\n[SEP] Selected: {selected_song_id}")
            
            # Check if already processed
            output_dir = get_song_directory(selected_song_id)
            if output_dir.exists() and any(output_dir.iterdir()):
                overwrite = input("[WARN] Song already processed. Overwrite? (y/n): ").strip().lower()
                if overwrite != 'y':
            ...
            if result:
                ...
                print("\n" + "="*70)
                print("[OK] SUCCESS! Song separated into 4 stems")
                print("="*70)
                print("\n[SEP] You can now use these stems for 16D spatial audio!")
                print(f"[SEP] Location: {output_dir}")
            else:
                print("\n[ERROR] Separation failed!")
        else:
            print("[ERROR] Invalid choice or cancelled")
