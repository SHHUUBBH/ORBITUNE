...
'indie': {
    ...
    def __init__(self):
        ...
        if self.api_available:
            ...
            if not GEMINI_API_KEY:
                print("[WARN] GEMINI_API_KEY not set. Using fallback genre detection.")
            print("[INFO] Genre detection: Using metadata-based fallback")
    ...
    def detect_genre(self, song_id: str) -> Tuple[str, float, Dict]:
        ...
        if not metadata_path.exists():
            print("[WARN] No metadata found, using default genre")
        ...
        print(f"\n[GENRE] Detecting genre for: {title}")
        print(f"[GENRE] Artist: {artist}")
        ...
        print(f"[GENRE] Genre detected: {profile['name']} (confidence: {confidence:.0%})")
        print(f"[GENRE] Profile: {profile['description']}")
